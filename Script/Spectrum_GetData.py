...
SIGNALS = ['V_DSA-V_DSB', 'V_MR', 'V_PRI', 'V_SEC', 'V_RECT', 'V_LOAD']
...
def com_prep():
    ...
    with p.Path(dir, 'instruments.log').open('w', encoding='cp1252') as log:
            ...
            for inst in [E3634A, E3631A, v33521A, v33510B, v34401A, EDU34450A, MSO7034B, EL34143A, FPC1000, ]:
                try:
                    inst.write('*CLS')
                    time.sleep(1)
                    ...
                    if inst == MSO7034B:
                        inst.write('*CLS')
    for inst in [v33510B, v33521A, EDU34450A, MSO7034B, EL34143A, FPC1000]:
...
def screen_copy():
    ...
    FPC1000.query_bin_block_to_file(f'MMEMory:DATA? {inst_filename}', f'{pc_filename}', append=False)
