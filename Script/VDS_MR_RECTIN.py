...
DATASET = 'VDS_VMR_RECTIN'

ANALOG_LABELS = ['VDS_A', 'VDS_B', 'V_MR', 'V_RECT_IN']
...
MSO7034B = rm.open_resource('MSO7034B')#('USB0::2391::5949::MY50340240::0::INSTR')
...
try:
    ...
    MSO7034B.write(':CHANnel4:PROBe %s' % ('X10'))


    MSO7034B.write(':CHANnel1:SCALe %G V' % (25.0))
    MSO7034B.write(':CHANnel2:SCALe %G V' % (25.0))
    MSO7034B.write(':CHANnel3:SCALe %G V' % (50.0))
    MSO7034B.write(':CHANnel4:SCALe %G V' % (50.0))
    ...
    for swp in SWEEPS:
        ...
        for var in swp.Points:
            ...
            MSO7034B.write(':MEASure:FREQuency %s' % ('CHANNEL3'))
            MSO7034B.write(':MEASure:VPP %s' % ('CHANNEL4'))

            vdsapp = MSO7034B.query_ascii_values(':MEASure:VPP? %s' % ('CHANNEL1')) [0]
            vdsbpp = MSO7034B.query_ascii_values(':MEASure:VPP? %s' % ('CHANNEL2')) [0]
            vdsamax = MSO7034B.query_ascii_values(':MEASure:VPP? %s' % ('CHANNEL1')) [0]
            vdsbmax = MSO7034B.query_ascii_values(':MEASure:VPP? %s' % ('CHANNEL2')) [0]
            mrfreq = MSO7034B.query_ascii_values(':MEASure:FREQuency? %s' % ('CHANNEL3')) [0]
            vmrpp =  MSO7034B.query_ascii_values(':MEASure:VPP? %s' % ('CHANNEL3')) [0]
            vrectpp = MSO7034B.query_ascii_values(':MEASure:VPP? %s' % ('CHANNEL4')) [0]
            vrectrms = MSO7034B.query_ascii_values(':MEASure:VRMS? %s' % ('CHANNEL4')) [0]
